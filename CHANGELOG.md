...
### Added
- **Distribution & Pitch section in the track template.** Each track `.md`
  now carries a `## Distribution & Pitch` section with DistroKid primary/
  secondary genre fields and a Spotify editorial pitch block (genres, moods,
  styles, instruments, similar artists, language, and a ≤500-character
  description). `release-director` fills this during Distribution Prep and
  includes guidance for writing the editorial pitch.
- **DistroKid genre/subgenre reference** in `metadata-by-platform.md`,
  documenting that a "Electronic" primary genre forces a mandatory subgenre
  (full closed list + Suno-style mapping hints). Template and release-director
  now carry the Subgenre field and the rule.
